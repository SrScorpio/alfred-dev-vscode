...
| VSIX | Empaquetado | `npm run package` genera el VSIX y `npx vsce ls --tree` lista únicamente runtime y metadatos | Cubierto manualmente |

## Casos priorizados por riesgo

| Prioridad | Área y escenario | Tipo | Resultado esperado |
| --- | --- | --- | --- |
| Crítica | Selector global: elegir un perfil y reabrir el selector | Integración / E2E | Se conserva la única preferencia global y el perfil guardado se identifica como seleccionado; los perfiles por agente quedan fuera de esta issue. |
