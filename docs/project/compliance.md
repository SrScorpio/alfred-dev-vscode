...
| Minimizacion y finalidad de datos | RGPD art. 5 | parcial | El TreeView lee solo `docs/project/status.md`; la memoria requiere opt-in, limita tamaño y sanitiza secretos; la galería guarda solo la elección confirmada. Falta inventario del tratamiento de marketplace/Copilot y aviso de privacidad del responsable. |
...
| Seguridad del tratamiento | RGPD art. 32 | parcial | Memoria local con escritura atómica, límites y sanitización; Secret Guard con avisos y hook explícito; CSP/nonce en galería; workspace trust y validación estricta en Ralph. El cifrado y controles del almacenamiento de configuración de VS Code no se han verificado. |
| Secretos y ejecución local | CRA / NIS2 | parcial | El scanner es puro y reutilizable, el hook solo se instala explícitamente y no bloquea toda edición. Ralph usa comandos fijos y no ejecuta cuerpos de issues/prompts; la API pública de Ralph no ofrece scheduler para paralelismo. |
