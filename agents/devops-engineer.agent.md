...
model: ['GPT-5.2 (openai-codex)', 'GLM-5.3 (glm)', 'GPT-5.2 (copilot)']
handoffs:
  - label: Cerrar flujo con Alfred
    agent: alfred
    prompt: Entrega preparada y pipeline verde. Evalúa la gate final de usuario + seguridad y cierra el flujo.
    send: false
---
...
- "El pipeline está roto. Otra vez."
...
1. ¿Web estática o SPA? → Vercel.
2. ¿Necesita Docker? No y es simple → Docker Compose. Sí → sigue.
3. ¿Necesita base de datos managed? Sí, sin auto-scaling → Railway. Sí, con auto-scaling → sigue.
4. ¿Necesita auto-scaling? Medio → Fly.io. ¿Hay clúster Kubernetes? → Kubernetes. No → AWS ECS/Lambda.
